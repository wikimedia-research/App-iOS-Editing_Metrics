...
p <- daily_editor_counts %>%
  ...
  labs(title = "Number of editors on the iOS app, by project",
       subtitle = "Including both anonymous and non-anonymous editors. Anonymous editors are identified by their IP address and user agent (when it's available*).",
       caption = "*About 20% of the edits' user agents are unavailable. For those edits, we identified their editors by IP address only.")
...
p <- all_ios_edits %>%
  ...
  labs(title = "Number of editors on the iOS app, by login status",
       subtitle = "Anonymous editors are identified by their IP address and user agent (when it's available*).",
       caption = "*About 20% of the edits' user agents are unavailable. For those edits, we identified their editors by IP address only.")
ggsave("all_ios_editors_bylogin.png", p, path = 'figures', units = "in", dpi = 300, height = 6, width = 10)
p <- all_ios_edits %>%
  mutate(is_anon = local_user_id == 0) %>%
  ...
  labs(title = "Number of Wikipedia editors on the iOS app, by login status",
       subtitle = "Anonymous editors are identified by their IP address and user agent (when it's available*).",
       caption = "*About 20% of the edits' user agents are unavailable. For those edits, we identified their editors by IP address only.")
ggsave("wikipedia_editors_bylogin.png", p, path = 'figures', units = "in", dpi = 300, height = 6, width = 10)
...
active_editors <- editor_month %>%
  ...
  inner_join({
    editor_month %>%
      filter(content_edits >= 5, month == floor_date(first_ios_edit_date, "month") %m+% months(1)) %>%
      group_by(month) %>%
      summarize(second_month_active_editors = n())
  }, by = 'month') %>%
 inner_join({
   editor_month %>%
     filter(content_edits >= 5, month != floor_date(first_ios_edit_date, "month") %m+% months(1), month != floor_date(first_ios_edit_date, "month")) %>%
     group_by(month) %>%
...
p <- active_editors %>%
  ...
  mutate(`User type` = factor(gsub("_", " ", `User type`),
                              levels = c("second month active editors", "first month active editors", "existing active editors"))) %>%
  ...
  labs(title = "Number of non-anonymous active iOS app editors, by user type",
       ...
       caption = "First month active iOS editors are users who made their first ever iOS app edit AND at least 5 wikipedia/wikidata content edits through the iOS app in the given calendar month.")
ggsave("active_editor_bynew.png", p, path = 'figures', units = "in", dpi = 300, height = 6, width = 11)
